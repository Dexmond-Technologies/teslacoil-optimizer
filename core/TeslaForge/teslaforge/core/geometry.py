...
class Wire(BaseModel):
    ...
    insulation_thickness_mm: float = Field(
        0.0, ge=0, description="Insulation thickness in mm"
    )
...
class PrimaryCoil(BaseModel):
    inner_radius_mm: float = Field(
        ..., gt=0, description="Inner radius of the primary coil in mm"
    )
    turns: float = Field(..., gt=0, description="Number of turns")
    wire: Wire
    turn_spacing_mm: float = Field(
        ..., ge=0, description="Edge-to-edge spacing between turns"
    )

    @property
    def outer_radius_mm(self) -> float:
        return self.inner_radius_mm + self.turns * (
            self.wire.outer_diameter_mm + self.turn_spacing_mm
        )
...
class SecondaryCoil(BaseModel):
    radius_mm: float = Field(
        ..., gt=0, description="Radius of the secondary coil form in mm"
    )
    ...
    turn_spacing_mm: float = Field(
        0.0,
        ge=0,
        description="Edge-to-edge spacing between turns (usually 0 for close-wound)",
    )
    ...
    def wire_length_m(self) -> float:
        return (
            2
            * math.pi
            * (self.radius_mm + self.wire.outer_diameter_mm / 2.0)
            * self.turns
        ) / 1000.0

    @property
    def dc_resistance_ohms(self) -> float:
        wire_radius_m = (self.wire.diameter_mm / 2.0) / 1000.0
        area = math.pi * wire_radius_m**2
        if area <= 0:
            return 0.0
        return (1.68e-8 * self.wire_length_m) / area

    def ac_resistance_ohms(self, freq_hz: float) -> float:
        from .physics import calc_ac_resistance

        wire_radius_m = (self.wire.diameter_mm / 2.0) / 1000.0
        pitch_m = (self.wire.outer_diameter_mm + self.turn_spacing_mm) / 1000.0
        return calc_ac_resistance(
            self.dc_resistance_ohms, wire_radius_m, freq_hz, pitch_m=pitch_m
        )


class Topload(BaseModel):
    major_diameter_mm: float = Field(
        ..., gt=0, description="Overall outer diameter in mm"
    )
