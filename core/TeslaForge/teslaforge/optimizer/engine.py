...
class TeslaForgeOptimizer:
    def __init__(
        self, target_power_w: float, primary_cap_f: float, target_fres_hz: float = None
    ):
        """
    ...
    def _generate_turn_coords(
        self,
        primary: PrimaryCoil,
        secondary: SecondaryCoil,
        pri_z_offset_m: float = 0.0,
    ):
        """
    ...
    def objective(self, trial: Trial):
        ...
        sec_wire_dia = trial.suggest_categorical(
            "sec_wire_dia", [0.2, 0.3, 0.4, 0.5]
        )  # mm
        ...
        top_major_mm = trial.suggest_float(
            "top_major_mm", top_minor_mm * 2 + 50.0, 1000.0
        )

        # Build models
        sec_wire = Wire(diameter_mm=sec_wire_dia, insulation_thickness_mm=0.02)
        secondary = SecondaryCoil(
            radius_mm=sec_radius_mm, turns=sec_turns, wire=sec_wire, turn_spacing_mm=0.0
        )
        ...
        topload = Topload(
            major_diameter_mm=top_major_mm, minor_diameter_mm=top_minor_mm
        )
    ...
    def local_tune_primary_turns(self, best_params: dict) -> float:
        ...
        def tune_fn(turns):
            ...
            primary = PrimaryCoil(
                inner_radius_mm=best_params["sec_radius_mm"]
                + best_params["pri_clearance_mm"],
                turns=t,
                wire=pri_wire,
            ...
            F_pri = calc_resonant_frequency(L_pri, self.primary_cap_f)
            return abs(F_pri - F_sec) / F_sec

        initial_guess = [best_params["pri_turns"]]
        bounds = [(1.0, 30.0)]
        res = minimize(tune_fn, initial_guess, method="SLSQP", bounds=bounds)
        return float(res.x[0])

    def optimize(self, n_trials=500, callbacks=None):
        # 1. Global Multi-Objective Optimization
        study = optuna.create_study(directions=["minimize", "minimize"])
        study.optimize(self.objective, n_trials=n_trials, callbacks=callbacks)
        return study
